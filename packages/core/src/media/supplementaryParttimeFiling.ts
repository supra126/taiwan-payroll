...
function premiumOf(year: number, r: SupplementaryParttimeRecord): number {
  return calcSupplementary(getYearData(year), { type: 'parttime', amount: r.amount }, 'round').premium;
}

export function generateSupplementaryParttimeFiling(input: SupplementaryParttimeFilingInput): SupplementaryBonusFilingResult {
  ...
  const headerRow = buildHeaderRow(unit, '63', start, end, records.length,
