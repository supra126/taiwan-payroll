...
@dataclass
class SupplementaryInterestFilingInput:
    year: int
...
def generate_supplementary_interest_filing(inp: SupplementaryInterestFilingInput) -> SupplementaryBonusFilingResult:
    recs = inp.records
    if not recs:
    ...
    premiums = [
        calc_supplementary(data, SupplementaryInput(type="interest", amount=int(r.amount)), "round").premium
        for r in recs
    ]
    u = inp.unit
